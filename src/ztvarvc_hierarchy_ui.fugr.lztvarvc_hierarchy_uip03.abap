...
CLASS application IMPLEMENTATION.
  ...
  METHOD on_f4.
    ...
    CASE screen_field.
      ...
      WHEN 'SELVAL-LOW' OR 'SELVAL-HIGH'.

        " Special handling required for selection screen popup
        app->screens->get_screen( '1500' )->on_f4( screen_field ).
  ...
  METHOD get_variable_preview.
    ...
    CASE attributes-tvarv_type.
      ...
      WHEN zcl_tvarvc=>kind-param.
        ...
        preview-value = variable_tools=>apply_output_conversion(
          int_val = variable_handler->get_parameter( attributes-tvarv_name )
          node    = node ).

        IF preview-value IS NOT INITIAL.
      ...
      WHEN zcl_tvarvc=>kind-selopt.
        ...
        preview-value_to = variable_tools=>apply_output_conversion(
          int_val = value-high
          node    = node ).
